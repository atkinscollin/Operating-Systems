...
int get (void *stream, pthread_t *p) {
   int ret;
	bool flag;
	ConnectEntry ce* = ((Stream*)stream)->ce, *ptr;
   ...
   pthread_mutex_lock(mutex);
	...
	if(ce& == NULL) {
		printf("get failed - no connections\n"); fflush(stdout);
		return -1;
	}

	//for- find thread p in ConnectEntry
	//  if - found thread p then break loop
	//if failed to find then give up
	for() if() break;
	if() {
		printf("get failed - not connected\n"); fflush(stdout);
		return -1;
	}

	//Wait as long as the buffer is empty or a request
	//  for a token not yet added is requested
   while(isEmpty(Q) && (request != lastpos) )
		pthread_cond_wait(empty, mutex);
	
	ret = peek(Q, ce->first - *first_pos);

	//checks if the token needs removed from buffer beginning
	flag = true;
