// Modifications by Collin Atkins, worked alongside Ryan Tackett
// 12/1/2015
...
/*
	Running the program:
	make
	./bbfs rootdir mountdir uid
	cat mountdir/file (do what you want)
...
// Determines if the user has access or not, and return T/F
bool HasAccess(struct bb_state* bb_data) {
	// Current user ID
	int gUid;
	gUid = getuid();
  
	// If the ID entered via arg does not match UID then fail
  	if (bb_data->uid != gUid) {
